...
color lowColor =    color(0,150,255);    // color of lowest elevation
color highColor =   color(255,150,0);    // ditto highest
...
void draw() {
  if (inputFilename != null) {
    ...
    output = createGraphics(outputWidth, int(outputWidth / ratio), PDF, outputFilename + "-ELEVATION.pdf");
    output.beginDraw();
    output.noStroke();
    prev = points.get(0);
    for (PVector p : points) {
      color startColor = lerpColor(lowColor, highColor, prev.z);
      color endColor = lerpColor(lowColor, highColor, p.z);
...
// how-to-draw-a-gradient-colored-line
void gradientLine(PVector p, PVector prev, color a, color b) {
  float deltaX = prev.x - p.x;
  float deltaY = prev.y - p.y;
  float inc = 1.0 / dist(p.x, p.y, prev.x, prev.y);
